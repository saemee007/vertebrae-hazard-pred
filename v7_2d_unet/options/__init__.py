...
def parse_option(print_option=True):    
    ...
    p.add_argument('--file_type', default='.nii.gz', type=str, help='nifti file type')
    p.add_argument('--dataset', default='snuh', type=str, help='boramae | snuh')
    
    # Data augmentation
    p.add_argument('--augmentation', default=True, type=str2bool, help='apply augmentation or not')
    p.add_argument('--rot_factor', default=0, type=float)
    p.add_argument('--scale_factor', default=0.1, type=float)
    p.add_argument('--bright_factor', default=0.25, type=float)
    # p.add_argument('--flip', default='False', type=str2bool)
    # p.add_argument('--trans_factor', default=0.1, type=float)

    # Input image
    p.add_argument('--in_res', default=512, type=int, help='input resolution using resize process (after center-cropping)')
    ...
    p.add_argument('--in_channels', default=1, type=int, help='number of input channels (input image types)')
    p.add_argument('--n_classes', default=3, type=int, help='number of output channels (output mask types)')
    ...
    p.add_argument('--class_weights', default='1.0,1.0,1.0', type=str, help='class weights of loss function')
    ...
    p.add_argument('--gpu_id', default="2", type=str)

    # Output directory
    p.add_argument('--exp', default='exp', type=str, help='checkpoint dir.')

    # Config file
    p.add_argument('--config', default=None, type=str, help='config file path')
