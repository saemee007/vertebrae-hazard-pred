...
class Modified3DUNet(nn.Module):
	...
	def forward(self, x):
		...
		# Level 1 localization pathway
		out = F.interpolate(out, size = context_4.size()[-3:])
		...
		# Level 2 localization pathway
		out = F.interpolate(out, size = context_3.size()[-3:])
		...
		# Level 3 localization pathway
		out = F.interpolate(out, size = context_2.size()[-3:])
		...
		# Level 4 localization pathway
		out = F.interpolate(out, size = context_1.size()[-3:])
		...
		ds1_ds2_sum_upscale = F.interpolate(ds1_ds2_sum_upscale, size = ds3_1x1_conv.size()[-3:])
