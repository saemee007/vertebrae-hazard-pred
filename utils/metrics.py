...
def get_classification_metrics(args, logging_dict):
    stats_dict = {}
    
    y = np.array(logging_dict["y"]).reshape(-1)
    probs = np.array(logging_dict["probs"])
    preds = (probs > 0.5).reshape(-1)
    probs = probs.reshape((-1, probs.shape[-1]))

    stats_dict["accuracy"] = accuracy_score(y_true=y, y_pred=preds)

    stats_dict["precision"] = precision_score(y_true=y, y_pred=preds)
    stats_dict["recall"] = recall_score(y_true=y, y_pred=preds)
    stats_dict["f1"] = f1_score(y_true=y, y_pred=preds)
    num_pos = y.sum()
    if num_pos > 0 and num_pos < len(y):
        stats_dict["auc"] = roc_auc_score(y, probs[:, -1], average="samples")
        stats_dict["ap_score"] = average_precision_score(
            y, probs[:, -1], average="samples"
        )
...
def get_survival_metrics(args, logging_dict):
    stats_dict = {}

    censor_times, probs, ys = (
    ...
    for followup in range(args.max_followup):
        ...
        fpr, tpr, roc_auc, acc, sen, spc = compute_auc_at_followup(
            probs, censor_times, ys, followup
        )
        stats_dict["{}_year_auc".format(min_followup_if_neg)] = roc_auc
        if args.save_roc_curve and (min_followup_if_neg in [2, 3, 5]):
            stats_dict["{}_year_fpr".format(min_followup_if_neg)] = fpr
            stats_dict["{}_year_tpr".format(min_followup_if_neg)] = tpr
        stats_dict["{}_year_acc".format(min_followup_if_neg)] = acc
        stats_dict["{}_year_sen".format(min_followup_if_neg)] = sen
        stats_dict["{}_year_spc".format(min_followup_if_neg)] = spc

    if np.array(ys).sum() > 0:
        stats_dict["c_index"] = concordance_index(
            logging_dict["censors"], probs, ys, args.censoring_distribution
        )
    else:
...
def compute_auc_at_followup(probs, censor_times, ys, followup, fup_lower_bound=-1):
    ys, censor_times = ys.ravel(), censor_times.ravel()

    def include_exam_and_determine_label(prob_arr, censor_time, y):
        valid_pos = y and censor_time <= followup and censor_time > fup_lower_bound
        valid_neg = censor_time >= followup
        included, label = (valid_pos or valid_neg), valid_pos
        return included, label

    probs_for_eval, y_for_eval = [], []
    for prob_arr, censor_time, y in zip(probs, censor_times, ys):
        include, label = include_exam_and_determine_label(prob_arr, censor_time, y)
        if include:
            if not np.isnan(prob_arr[followup]): 
                probs_for_eval.append(prob_arr[followup])
                y_for_eval.append(label)
            else:
                continue

    try:
        fpr, tpr, threshold = roc_curve(y_for_eval, probs_for_eval)
        optical_idx = np.argmax(tpr - fpr)
        optical_threshold = 0.5 if (np.isnan(tpr).mean()==1) or (np.isnan(fpr).mean()==1) else threshold[optical_idx]
        tn, fp, fn, tp = confusion_matrix(y_for_eval, probs_for_eval > optical_threshold).ravel()
        roc_auc = roc_auc_score(y_for_eval, probs_for_eval, average="samples")
        acc = accuracy_score(y_for_eval, probs_for_eval > optical_threshold)
        sen = tp / (tp + fn)
        spc = tn / (tn + fp)
        # ap_score = average_precision_score(
            # y_for_eval, probs_for_eval, average="samples"
        # )
        # precision, recall, _ = precision_recall_curve(y_for_eval, probs_for_eval)
    ...
    return fpr, tpr, roc_auc, acc, sen, spc
...
def get_censoring_dist(train_dataset):
    times, event_observed = zip(*[(d["time_at_event"], d["y"])  for d in train_dataset])
