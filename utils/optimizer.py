...
def extract_weight_method_parameters_from_args():
    ...
    weight_methods_parameters.update(
        dict(
            nashmtl=dict(
                update_weights_every=1, # args.update_weights_every,
                optim_niter=20, # args.nashmtl_optim_niter
            ),
            stl=dict(main_task=1), # args.main_task
            cagrad=dict(c=0.4), # args.c
            dwa=dict(temp=2.0), # args.dwa_temp
        )
    )
